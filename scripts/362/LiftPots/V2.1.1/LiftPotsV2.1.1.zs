...
namespace LiftObjects
{
	...
	CONFIGB USE_PTM = false; //Whether or not to include the Player Tile Modifier in custom sprites.
	...
	itemdata script PowerBracelet //start
	{
		 ...
		 * Flags[5]: If checked, the 'next' combo will always be used.
		 *                  - Bush->Next will use the next combo regardless.
		 * Flags[6]: If checked, the item will be the room's "Special Item", only
		 ...
		 * Flags[7]: If checked, the item will appear when the object breaks on impact,
		...
		void run(int throwspeed, int upthrow, int liftDelay, int COMBO_PLAYER_HOLDING_ANIM)
		{
			...
			while(true)
			{
							...
							if(canLift)
							{
								...
								if(liftDelay)
								{
									...
									for(int q = 0; q < liftDelay; ++q)
									{
										if(pcombo)
										{
											Hero->ScriptTile = pcombo->Tile + (USE_PTM ? Hero->TileMod : 0);
										}
				...
				if(held_combo)
				{
					int lastScreen = Game->GetCurScreen(), lastDMap = Game->GetCurDMap();
					while(held_combo)
					{
						...
						KillButton(CB_THROW);
						Waitdraw();
						int xoffs = Hero->DrawXOffset + (Hero->Action==LA_SCROLLING ? Game->Scrolling[SCROLL_NX] : 0);
						int yoffs = Hero->DrawYOffset - (Hero->Z + Hero->DrawZOffset) + (Hero->Action==LA_SCROLLING ? Game->Scrolling[SCROLL_NY] : 0);
						Screen->FastCombo(HELD_DRAW_LAYER, Hero->X+xoffs, Hero->Y-8+yoffs, held_combo->ID, cset, OP_OPAQUE);
						Waitframe();
						unless(this->Flags[FLAG_CROSSSCREEN] || (lastScreen == Game->GetCurScreen() && lastDMap == Game->GetCurDMap()))
						{
							held_combo = NULL;
