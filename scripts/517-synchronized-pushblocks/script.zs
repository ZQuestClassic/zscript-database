const int CMB_SYNC_TALL_PUSHBLOCK_SOLID = 1; //ID of combo used to mimic solidity. It must be fully solid and fully transparent.

const int SFX_SYNC_TALL_PUSHBLOCK_MOVE = 50; //Sound to play when moving a block.
const int SFX_SYNC_TALL_PUSHBLOCK_STUCK = 16;//Sound to play when pushblock gets stuck.

const int SYNC_ICEBLOCK_DAMAGE = 4;//BE VERY CAREFUL WITH SYNCHED ICEBLOCKS. IF ONE HITS LINK FROM BEHIND, HE WILL TAKE DAMAGE, in 1/4ths of heart!

//Synchronized Pushable blocks. If Link pushes one, others move in the same direcction, if they can.
...
//SyncPushRender
//Renderer for SyncPush blocks
//Place, gridsnap, and align on bottom with large pushblock.
//D0 - ID of pushblock FFC

ffc script LargeSyncPushBlock{
	void run (int weight, int dirs, int flags, int syncID){
		int origdata= this->Data;
		int cmb=origdata;
		int str[] = "SyncPushRender";
		...
		int ucset[16];
		for (int i=0;i<16;i++){
			ucmb[i] = CMB_SYNC_TALL_PUSHBLOCK_SOLID;
			ucset[i] = this->CSet;
		}
		SyncPushReplaceCombosUnderFFC(this, ucmb, ucset);
		int pushcounter = 0;
		bool linkmove = false;
		this->InitD[7] = -1;
		this->InitD[5] = -1;
		...
		while(true){
			if (this->InitD[7]<0){				
				if (SyncPushBlockIsPushed(this,Link->Dir, 6)){
					pushcounter++;
				}
				else pushcounter = 0;
				if (pushcounter>=8 || this->InitD[5]>=0){
					if (SyncPushBlockCanBePushed(this, Cond(this->InitD[5]>=0,this->InitD[5],Link->Dir)) && !EnemiesAlive()){
						NoAction();
						this->InitD[7] = Cond(this->InitD[5]>=0,this->InitD[5],Link->Dir);
						Game->PlaySound(SFX_SYNC_TALL_PUSHBLOCK_MOVE);
						movecounter = 16;
						...
						for(int i=1;i<=32;i++){
							...
							if (!SyncPushBlockCanBePushed(n, Link->Dir)) continue;
				...
				if (movecounter==0){
					if (this->InitD[5]>=0 && LinkCollision(this) && (flags&4)>0 && SYNC_ICEBLOCK_DAMAGE>0){
						eweapon e = FireEWeapon(EW_SCRIPT10, Link->X+InFrontX(Link->Dir, 12), Link->Y+InFrontY(Link->Dir, 12), 0, 0, SYNC_ICEBLOCK_DAMAGE, -1, -1, EWF_UNBLOCKABLE);
						e->Dir = Link->Dir;
						e->DrawYOffset = -1000;
						SetEWeaponLifespan(e, EWL_TIMER, 1);
						SetEWeaponDeathEffect(e, EWD_VANISH, 0);
					}
					if ((flags&4)>0 && SyncPushBlockCanBePushed(this, this->InitD[7])) movecounter=8;
					else{
						if ((flags&1)>0){
							Game->PlaySound(SFX_SYNC_TALL_PUSHBLOCK_STUCK);
							this->InitD[1]=0;
						}
						SyncPushReplaceCombosUnderFFC(this, ucmb, ucset);
						if (linkmove){
						...
						this->InitD[5]=-1;
						pushcounter = 0;
						if ((flags&8)==0)SyncPushTriggerUpdate(this,ucset);
...
bool SyncPushBlockIsPushed(ffc this, int dir, int margin){
...
bool SyncPushBlockCanBePushed(ffc this, int dir){
...
void SyncPushReplaceCombosUnderFFC(ffc this, int ucmb, int ucset){
	int x1 = this->X+1;
	int y1 = this->Y+1;
...
void SyncPushTriggerUpdate(ffc f, int ucset){
	f->InitD[6]=1;
	for (int i=0;i<16;i++){
		int x = f->X+(i%4)*16;
