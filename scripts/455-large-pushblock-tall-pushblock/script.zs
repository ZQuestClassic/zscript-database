const int CMB_TALL_PUSHBLOCK_SOLID = 1; //ID of combo used to mimic solidity. It must be fully solid and fully transparent.

const int SFX_TALL_PUSHBLOCK_MOVE = 50; //Sound to play when moving a block.
const int SFX_TALL_PUSHBLOCK_STUCK = 16;//Sound to play when pushblock gets stuck.

//Large Pushable blocks + Tall pushables. 
//Set up solid combo with blank tile for CMB_TALL_PUSHBLOCK_SOLID constant.
//Effect Width and Height used. If TileHeight is greater than EffectHeight / 16, then the script calls TallPush Renderer, so FFC sprite rendering is aligned to bottom edge of FFC`s hitbox.
...
ffc script LargeTallPushBlock{
	void run (int weight, int dirs, int flags){
		...
		int ucset[16];
		for (int i=0;i<16;i++){
			ucmb[i] = CMB_TALL_PUSHBLOCK_SOLID;
			ucset[i] = this->CSet;
		}
		TallPushReplaceCombosUnderFFC(this, ucmb, ucset);
		int pushcounter = 0;
		this->InitD[7] = -1;
		int movecounter = 0;
		for (int i=0;i<176;i++){
		...
		while(true){
			if (this->InitD[7]<0){
				if (TallPushBlockIsPushed(this,Link->Dir, 6))pushcounter++;
				else pushcounter = 0;
				if (pushcounter>=8){
					if (TallPushBlockCanBePushed(this, Link->Dir, weight, dirs) && !EnemiesAlive()){
						this->InitD[7] = Link->Dir;
						Game->PlaySound(SFX_TALL_PUSHBLOCK_MOVE);
						movecounter = 16;
				...
				if (movecounter==0){
					if ((flags&4)>0 && TallPushBlockCanBePushed(this, this->InitD[7], 0, dirs)) movecounter=8;
					else{
						if ((flags&1)>0){
							Game->PlaySound(SFX_TALL_PUSHBLOCK_STUCK);
							dirs=0;
						}
						TallPushReplaceCombosUnderFFC(this, ucmb, ucset);
						this->InitD[7]=-1;
						if ((flags&8)==0)TallPushTriggerUpdate(this,ucset);
...
bool TallPushBlockIsPushed(ffc this, int dir, int margin){
...
bool TallPushBlockCanBePushed(ffc this, int dir, int weight, int dirs){
	if (dir==DIR_UP){
...
void TallPushReplaceCombosUnderFFC(ffc this, int ucmb, int ucset){
	int x1 = this->X+1;
	int y1 = this->Y+1;
...
void TallPushTriggerUpdate(ffc f, int ucset){
	f->InitD[6]=1;
	for (int i=0;i<16;i++){
		int x = f->X+(i%4)*16;
