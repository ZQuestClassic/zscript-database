...
ffc script SplitOnDeath {
	void run(int enem_id, int splits_into, int number_of_splits, int dist_flux, int split_sfx, int gleeok_override ){
		npc n[2]; int a[10];
...
ffc script SplitOnHit {
	void run(int enem_id, int splits_into, int number_of_splits, int dist_flux, int split_sfx, int dying_no_split, int gleeok_override ){
		npc n[2]; int a[10];
		Waitframes(5); //Enemies require five frames, to spawn.
		  // If a 'No Return' flag is set, and there are no enemies on the screen, 
		 //  cleanly exit the script and make the ffc slot available.
