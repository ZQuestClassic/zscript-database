...
//D2 - add together: 1 - gets stuck after 1 push, 2 - gets stuck when landing on trigger (disarms timer), 4 - icy block, continue moving in direction until hitting obstacle, 8 - ignore triggers, 16 - require triggers with same CSet as FFC`s Cset. 32 - Start timer at screen init, 64 - don`t fail secrets on explosion on trigger 128 - superbomb explosion
			...
			if (impulse<0){
				...
				if (movecounter==0){
					if ((flags&4)>0 && TimedPushBlockCanBePushed(this, impulse, 0, dirs)) movecounter=8;
						...
						TimedPushReplaceCombosUnderFFC(this, ucmb, ucset);
						impulse=-1;
						if ((flags&8)==0)TimedPushTriggerUpdate(this);
...
void TimedPushTriggerUpdate(ffc f){
	f->InitD[6]=1;
	for (int i=0;i<16;i++){
		int x = f->X+(i%4)*16;
