...
void StatusEffectsUpdate1(){
	...
	if (STATUS_STATE[10]>0){
		//If you want to add more slots for status effects, just duplicate this piece of code or
		//advanced template above in this scope as many times, as you want.
...
//This FFC script curses any character with given status effect as long as the script is running.
//FFC carryover can be enabled to expand cursed area beyond one screen.
//Place FFC anywhere in the screen.
...
ffc script CursedZone{
	void run(int status, int nullifier){
		while (true){
			if (Link->Item[nullifier]){
				if(IsActive(status))RemoveStatusEffect(status, false);
			}
			if (STATUS_STATE[status]==STATUS_STATE_OFF) InduceStatusEffect (status, 2, nullifier, false);
			else STATUSTIMERS[status] = 2;
			Waitframe();
		}
	}
}